...
while(True):
    ...
    num0fLabels, img_label, state, centroids = cv.connectedComponentsWithStats(img_mask)

    for idx, centroid in enumerate(centroids):
        if state[idx][0] == 0 and state[idx][1] == 0:
            continue
        if np.any(np.isnan(centroid)):
            continue

        x, y, width, height, area = state[idx]
        centerX,centerY = int(centroid[0]), int(centroid[1])

        if area > 50:
            cv. circle(img_color,(centerX, centerY), 10, (0, 0, 255), 10)
            cv. rectangle(img_color, (x,y), (x+width,y+height), (0,0,225))
